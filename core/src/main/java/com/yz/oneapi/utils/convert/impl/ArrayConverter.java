...
import com.yz.oneapi.utils.ArrayUtil;
import com.yz.oneapi.utils.ByteUtil;
...
public class ArrayConverter extends AbstractConverter<Object> {
	...
	private Object convertObjectToArray(Object value) {
		...
		if (value instanceof List) {
			...
			final List<?> list = Lists.toList((Iterable<?>) value);
			result = Array.newInstance(targetComponentType, list.size());
			for (int i = 0; i < list.size(); i++) {
				Array.set(result, i, convertComponentType(list.get(i)));
			}
			...
			// 可循环对象转数组，可循环对象无法获取长度，因此先转为List后转为数组
			final List<?> list = Lists.toList((Iterator<?>) value);
			result = Array.newInstance(targetComponentType, list.size());
			for (int i = 0; i < list.size(); i++) {
				Array.set(result, i, convertComponentType(list.get(i)));
			}
	...
	private Object convertComponentType(Object value) {
		return AutoConvert.convertWithCheck(this.targetComponentType, value, null, this.ignoreElementError);
	}
	// -------------------------------------------------------------------------------------- Private method end
}
