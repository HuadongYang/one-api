...
public class SnowflakeSequence implements Sequence {
    ...
    // 修改雪花算法为单利模式
    private volatile static SnowflakeSequence singleton = null;

    private static Map<String, SnowflakeSequence> singletonMap = new ConcurrentHashMap<>();

    public static SnowflakeSequence getSingleton(long workId, long datacenterId) {
        return singletonMap.computeIfAbsent(workId + "_" + datacenterId, (k) -> new SnowflakeSequence(workId, datacenterId));
    }
