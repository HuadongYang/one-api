...
import com.yz.oneapi.utils.convert.ConvertException;
...
public class DateConverter extends AbstractConverter<Date> {
	...
	protected java.util.Date convertInternal(Object value) {
		if (value == null || (value instanceof CharSequence && StringUtil.isBlank(value.toString()))) {
			return null;
		}
		if (value instanceof TemporalAccessor) {
			...
			final DateTime dateTime = StringUtil.isBlank(this.format) //
	...
	private java.util.Date wrap(DateTime date) {
		...
		throw new UnsupportedOperationException(StringUtil.format("Unsupported target Date type: {}", this.targetType.getName()));
	}
